...
Journal checklist blocks support the portable task markers `TODO`, `DOING`, and `DONE`. Use the Journal slash menu to create one, click its visible TODO/DOING status to cycle it, click its checkbox to complete or reopen it, or press `Cmd+Enter` on macOS or `Ctrl+Enter` on Windows and Linux while the text cursor is in the task. Every unfinished `DOING` task remains at its original Journal location and is also projected at the bottom of the newest Journal in that workspace. Marking a projected task done updates its original block.
