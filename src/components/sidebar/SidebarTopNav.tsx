import { Archive, CalendarDots, FileText, Tray } from '@phosphor-icons/react'
...
export function SidebarTopNav(options: SidebarTopNavProps) {
  const { selection, onSelect, showInbox, inboxCount, activeCount, journalCount, archivedCount, onOpenJournals, locale = 'en', loading = false } = options
  return (
    <div className="border-b border-border" data-testid="sidebar-top-nav" style={{ padding: '4px 6px' }}>
      ...
      <NavItem
        icon={CalendarDots}
        label={translate(locale, 'sidebar.nav.journals')}
        count={journalCount}
        countLoading={loading}
        isActive={isSelectionActive(selection, {
          kind: 'filter',
        ...
        badgeStyle={{ background: 'var(--muted)' }}
        activeBadgeClassName="bg-primary text-primary-foreground"
        onClick={onOpenJournals ?? (() => onSelect({ kind: 'filter', filter: 'journals' }))}
      />
      <NavItem
        icon={Archive}
