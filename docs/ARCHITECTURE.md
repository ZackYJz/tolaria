...
Journals are date-addressed outline notes stored as `journals/YYYY-MM-DD.md` with `type: Journal`. `src/utils/journals.ts` owns local-date formatting, strict filename recognition, ordering, and day navigation. The Journals sidebar destination opens today's entry, creating it only when missing, and the editor date navigator opens previous, today, or next entries through the same resolver. Journal events report navigation source and creation state only, never a date, path, title, or content. See [ADR-0180](./adr/0180-outline-display-and-date-addressed-journals.md).
