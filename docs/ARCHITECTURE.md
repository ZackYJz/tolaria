...
Outline notes use the same BlockNote and Markdown persistence pipeline with `_display: outline`; the display mode adds compact list spacing and visible nesting guides without introducing another editor or serializer. Existing list nesting, collapse, and fenced-code schema behavior therefore applies inside outlines, including code blocks rendered beneath nested list items. The new-note menu and command palette can create outline notes explicitly while the existing document shortcut remains a normal text note.

Journals are date-addressed outline notes stored as `journals/YYYY-MM-DD.md` with `type: Journal`. `src/utils/journals.ts` owns local-date formatting, strict filename recognition, ordering, and day navigation. The Journals sidebar destination opens today's entry, creating it only when missing, and the editor date navigator opens previous, today, or next entries through the same resolver. `src/utils/journalTasks.ts` recognizes portable `TODO`, `DOING`, and `DONE` markers in legacy list blocks and Markdown checklist blocks, synchronizing the checkbox marker during exact-source status replacement. Journal slash commands explicitly create checklist tasks; ordinary typed status words never change a block's type. Checklist tasks render a Logseq-style outline marker, checkbox, and clickable TODO/DOING control, while DONE derives checked state and hides the status label. Only the newest Journal in a workspace renders the `DOING` projection; it reads all Journal sources, keeps every task at its original path and line, and routes completion back through normal note persistence. Journal events report navigation source, creation state, or task status transitions only, never a date, path, title, or content. See [ADR-0180](./adr/0180-outline-display-and-date-addressed-journals.md).
...
- **Editor** (flex, fills remaining space): Single note open at a time (no tabs — see ADR-0003). Breadcrumb bar with filename controls, read-only legacy display-title context when a no-H1 note's title differs from its filename, word count, rich-editor width toggle, and the secondary-overflow Table of Contents action, BlockNote rich text editor with wikilink support, Markdown-compatible inline/display math rendering, editable Obsidian/GFM-style blockquote callouts, slash-inserted toggle blocks backed by portable `<details>/<summary>` Markdown, first-class Mermaid diagram blocks, sandboxed fenced HTML blocks, markdown-safe formatting controls, and schema-backed fenced code block highlighting via `@blocknote/code-block` plus lazy direct `@shikijs/langs` registrations for missing common grammars. Tolaria overlays BlockNote's native code-language selector with a block-ID-keyed shadcn picker so transient read-only mounts cannot leave a code block's visible selector disabled. Rich Markdown pages append `PageReferences` inside the same width-constrained scroll surface: linked references reuse Neighborhood's relationship grouping, while unlinked mentions are discovered through the existing body-only vault search only when the page footer approaches the viewport. Candidate bodies are read in bounded batches so text references show the exact matching source line and outline references preserve the matched item plus its descendant subtree. Linked references start expanded, unlinked mentions start collapsed, stale searches are discarded on note switches, and the computed section is excluded from note PDF exports. The highlight control is split: its primary action applies default yellow, its caret opens the localized color menu, and a highlighter button appears beside the active highlight boundary for in-place recoloring. Callout bodies retain rich inline marks and links, known aliases use semantic app colors, unknown types use neutral styling, and `+`/`-` fold markers control only the initial disclosure state while remaining unchanged in Markdown. Sandboxed HTML blocks resolve renderer-owned `{{...}}` vault expressions against current-note properties, external note scalar properties, single sheet cells, raw body-line references, and `json(...)` structured-data helpers before the sanitizer/iframe boundary. Scripts are blocked unless the fence explicitly declares `scripts="sandboxed"`; installed builds serve that sanitized document through the private `tolaria-html-block` Tauri protocol so it receives a dedicated response-header CSP without weakening the app window policy. The sandbox grants only opaque-origin inline script execution while keeping same-origin, network, workers, forms, nested frames, parent DOM access, Tauri IPC, and top navigation unavailable. Can toggle to diff view (modified files), raw CodeMirror view, or a wide rich-editor reading surface with preserved side margins; raw CodeMirror remains full-width and unaffected by note width mode. Standalone HTML/HTM files use the same editor surface for an opaque-origin `HtmlFilePreview`: active content and remote passive loads are removed, vault-contained local resources are rewritten to scoped asset URLs, and the existing breadcrumb source action or `Cmd/Ctrl+\` opens CodeMirror for direct editing. Their breadcrumb keeps file/source/export actions while suppressing Markdown-only favorite, organized, neighborhood, width, table-of-contents, and archive controls. Raw CodeMirror chooses syntax highlighting from the file extension for Markdown, YAML, JSON, Python, SQL, JavaScript, and TypeScript files, highlights HTML inside fenced `html` blocks, and keeps unknown text files plain. Inline rich-editor images open in a localized shadcn lightbox on double-click while normal single-click BlockNote selection remains untouched, and tiny tracking-style images are ignored. Binary image, audio, video, and PDF files render through `FilePreview` as ordinary vault files using Tauri asset URLs; editor-embedded audio and video use the same scoped asset sources through the CSP `media-src` allow-list, while packaged PDF previews require scoped asset sources in both `object-src` and `frame-src` because the webview PDF renderer uses a nested frame context. Linux AppImage builds ask the native runtime whether audio/video should fall back to external-open controls before mounting webview media elements. External-open actions call `open_vault_file_external` so the target is validated against the active vault before the native default app opens it. Unsupported/broken binaries show explicit fallback states and keyboard focus returns to the note list on `Escape`. Decomposed into `Editor` (orchestrator), `EditorContent`, `HtmlFilePreview`, `FilePreview`, `EditorRightPanel`, `TableOfContentsPanel`, `PageReferences`, `SingleEditorView`, with hooks `useDiffMode`, `useEditorFocus`, and `useEditorSave`, plus the `useRawMode`/`RawEditorView` pair for raw source editing. Rich BlockNote input and raw CodeMirror input both route typed `->`, `<-`, and `<->` through the shared `src/utils/arrowLigatures.ts` resolver so arrow ligatures stay consistent across mode switches while escaped ASCII sequences remain literal. Rich-editor Markdown input transforms for arrows, inline math, and `==highlight==`/`==🔴highlight==` share one capture-phase `beforeinput` execution path in `src/components/richEditorInputTransform.ts` and are composed by `src/components/richEditorInputTransformExtension.ts`. Navigation history (Cmd+[/]) replaces tabs.
  ...
  Callouts cover every default Obsidian type and alias for Markdown compatibility while presenting a Notion-style full-width tinted surface with inline content, no type-title row, and a reusable emoji picker on the leading icon. The `/` command inserts the default note callout directly; selected emoji remain portable through the marker title. Multiline callout serialization keeps raw quote lines free of generated trailing hard-break backslashes.
