...
Journals are date-addressed outline notes stored as `journals/YYYY-MM-DD.md` with `type: Journal`. `src/utils/journals.ts` owns local-date formatting, strict filename recognition, ordering, and day navigation. The Journals sidebar destination opens today's entry, creating it only when missing, and the editor date navigator opens previous, today, or next entries through the same resolver. `src/utils/journalTasks.ts` recognizes portable `TODO`, `DOING`, and `DONE` markers at the start of Markdown list blocks and owns exact-source status replacement. Only the newest Journal in a workspace renders the `DOING` projection; it reads all Journal sources, keeps every task at its original path and line, and routes completion back through normal note persistence. Journal events report navigation source, creation state, or task status transitions only, never a date, path, title, or content. See [ADR-0180](./adr/0180-outline-display-and-date-addressed-journals.md).
