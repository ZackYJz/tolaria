...
`src/utils/journalTasks.ts` is the task-projection module. Its interface parses both legacy list-block workflow markers and checklist-backed markers, collects `DOING` tasks, computes status transitions, and replaces a status only when the source line still matches the projection snapshot. `JournalDoingTasks` loads Journal content only for the active workspace and renders only beneath that workspace's newest Journal. The projection never copies or moves Markdown; completion is persisted against the source note, including closed tabs. `journalTaskShortcutExtension` owns checklist task interaction inside Journals: slash commands explicitly create the checklist block, `TODO` and `DOING` render as persistent clickable status controls beside the checkbox, and `DONE` owns the checked state. Plain typed status words and legacy list blocks keep their original block type. The same extension owns the Journal-only text-cursor shortcut for `TODO → DOING → DONE`, while explicit whole-block selection keeps the existing collapse shortcut.
...
- `useCompositionAwareEditorChange()` defers rich-editor persistence during IME composition and schedules its pending flush after ProseMirror's own `compositionend` reconciliation microtask. Saving can therefore observe only the committed CJK text instead of persisting the preedit string and refreshing a nested list before the commit lands.
- `createImeCompositionCompatibilityExtension()` leaves standard composition ownership with BlockNote and ProseMirror instead of installing capture-phase keyboard or `beforeinput` guards. Its macOS Tauri adapter handles the WKWebView-specific case where ProseMirror suppresses an `Enter` immediately following `compositionend`: it routes that key through the editor's existing `handleKeyDown` commands, and defers a composing `Enter` until composition reconciliation only when WebKit has not already moved to another block. Separate ProseMirror plugins preserve nested-list DOM during WebKit composition and reconstruct an IME-committed slash command such as `/table` after reconciliation.
...
- The `/` slash menu remains the supported path for markdown-safe block transformations such as headings, quotes, list blocks, Mermaid diagrams, sandboxed HTML blocks, and whiteboards. Journal editors additionally expose searchable `TODO`, `DOING`, and `DONE` commands that convert the current block in place to a checklist task, preserve its inline content, synchronize checkbox completion, and return the text cursor to that task. `/date`, `/time`, and `/datetime` resolve the user's current local calendar values to plain Markdown text (`YYYY-MM-DD`, `HH:mm`, and `YYYY-MM-DD HH:mm`) at insertion time. Tolaria filters out BlockNote's toggle-heading and toggle-list variants because those do not map cleanly to the markdown note model.
...
`webBookmark` stores `url`, `title`, `description`, `image`, and `favicon` as string props. `bookmarkMarkdown.ts` preserves that snapshot in a `bookmark` JSON fence through the canonical durable Markdown pipeline, including empty placeholders and nested blocks. Opening a note never refetches metadata; explicit create/edit fetches update the snapshot. Invalid JSON remains editable source, and URL fields accept only HTTP/HTTPS without credentials.
